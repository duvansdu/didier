<div id="subir_acta" style="width:200px; margin-top:10px; display:inline-block;">
    ...
    <input type="text" id="txt_acta_individual" style="margin-top:10px; margin-bottom:10px; width:200px; height:30px">
    		...
    		<button id="btn_documento4" class="btn_documento">Enviar</button>

</div>
<div id="file">
<input type="file" id="txt_documento" accept="application/pdf" onChange="cambiar_acta(this.value)" style="display:inline-block">
</div>
<p style="font-family:Arial, Helvetica, sans-serif; font-size:18px;">Ingresar los datos para la propuesta</p>

<div id="nuevos_datos" style="width:400px; margin:0px auto">
	<textarea id="titulo" style="width:400px; height:100px; padding-left:10px; font-family:Arial, Helvetica, sans-serif; font-size:16px; resize:none;" placeholder="Titulo de la Propuesta"></textarea>
    
    <select id="modalidad" class="selec_mul" style="margin-top:15px; margin-bottom:15px;">
    	<option value="0">Selecione Modalidad</option>
        <?php  
			while($f=mysqli_fetch_array($modaliad)){?>
            <option value="<?php echo $f['id_modalidad']?>"><?php echo $f['modalidad']?></option>
        ...
        <select id="programa" class="selec_mul">
    	<option value="0">Selecione Programa</option>
        <?php  
			while($f=mysqli_fetch_array($programa4)){?>
