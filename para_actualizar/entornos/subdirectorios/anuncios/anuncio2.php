...
<div id="derecho"><h1>Plataforma Pegaso</h1>

    	<p class="parrafo">Esta plataforma le dará un control de sus documentos durante todos los procesos ya sean propuestas, proyectos como también le mostrará conceptos emitidos por los docentes tales como actas u observaciones.</p>
        
        <p class="parrafo">La plataforma Pegaso le dará la opción de ver anuncios emitidos por el administrador, anuncios que deberá leer para tener en cuenta las programaciones; como las fechas límites para subir sus documentos ya que el administrador contará con el privilegio de otorgarle las fechas en que se habilitara la plataforma para que los estudiantes suban sus archivos. Esto es un control para que los estudiantes puedan hacer sus documentos con anticipación.</p>
        
        <p class="parrafo">
        	Al lado izquierdo podrás ver unos archivos, los cuales se mostrarán cuando miembros del comité o coordinador hayan emitido conceptos sobre su documento.
        </p>
...
<?php 
  ...
  <?php 	while($f=mysqli_fetch_array($corregir)){
	  			if($f['revisado']==1){$style='style="background:#099; height:120px; margin:auto; padding-top:10px; border-radius:20px; width:55%; cursor:pointer"';}
				else{$style='';}
		?>
 ...
 if(mysqli_num_rows($cartas)>0){?>
     ...
     <h3>Cartas</h3> 
<?php	 while($f=mysqli_fetch_array($cartas)){
 ...
 <h3>Conceptos</h3>
<?php  if(mysqli_num_rows($concepto)>0){
...
if(mysqli_num_rows($concepto)==0 && mysqli_num_rows($obser_coor)==0){?>
