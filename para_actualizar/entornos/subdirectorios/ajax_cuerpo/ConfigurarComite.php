...
$datosComite=mysqli_fetch_assoc(mysqli_query($link, "SELECT tb_usuario.nombre,
						  ...
						  FROM (pegasoun_proyecto.tb_comite tb_comite
								INNER JOIN pegasoun_proyecto.tb_usuario tb_usuario
								   ON (tb_comite.id_usuario = tb_usuario.id_usuario))
...
<table align="center">
    ...
    <tr height="50px">
    	<td colspan="2" align="center"><button class="botonConfi" onClick="ConfigurarComite($('#NComite').val(), $('#AComite').val(), $('#TComite').val(), $('#CComite').val(), $('#EComite').val(), $('#UComite').val(), $('#PComite').val(), $('#RPComite').val(),'<?php echo $_SESSION["user"]?>')">Modificar</button></td>
