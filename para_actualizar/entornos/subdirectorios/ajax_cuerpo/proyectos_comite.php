...
function obtener_total_sql($opcion, $id_comite, $link){
	...
	switch($opcion){
			...
			case 5:{
					...
					$consul=mysqli_query($link,"SELECT tb_trabajo.codigo_trabajo,
											   tb_trabajo.titulo,
											   tb_trabajo.id_trabajo,
											   tb_trabajo.id_programa
										  FROM    pegasoun_proyecto.tb_comite_trabajo tb_comite_trabajo
											   JOIN
												  pegasoun_proyecto.tb_trabajo tb_trabajo
											   ON (tb_comite_trabajo.id_trabajo = tb_trabajo.id_trabajo)
										 WHERE tb_comite_trabajo.id_comite = '".$id_comite."' AND tb_comite_trabajo.estado = '1'");
...
function desarrollo_tecnologico($id_comite, $link){
  ...
  FROM    (   (   (   (   pegasoun_proyecto.tb_comite_trabajo tb_comite_trabajo
                       JOIN
                          pegasoun_proyecto.tb_trabajo tb_trabajo
                       ON (tb_comite_trabajo.id_trabajo =
                              tb_trabajo.id_trabajo))
                   JOIN
...
$html='';
			...
			while($g=mysqli_fetch_array($a)){
				...
				$html=proyectos_desa_gestion($g['anteproyecto'], $g['codigo_trabajo'], $g['titulo'], $g['nombre'], $g['id_trabajo'], $comite['id_comite']);
...
//Formulacion de una Investigacion				
